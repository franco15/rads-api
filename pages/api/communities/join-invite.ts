...
import { IInvite } from "@/lib/interfaces";
...
async function joinCommunity(req: NextApiRequest, res: NextApiResponse<Data>) {
	try {
		...
		if (!invite) return res.status(404).json({ message: "inivte not found" });

		// check if user already in community
		const communityIn = await prisma.userCommunity.findFirst({
			where: {
